        ...
        LBFMRecommendProvider.request(LBFMRecommendAPI.recommendList) { (result) in
            if case let .success(response) = result{
                ...
                if let mappedObject = JSONDeserializer<LBFMHomeRecommendModel>.deserializeFrom(json: json.description){// 从字符串转换为对象实例
                    ...
                    if let recomment = JSONDeserializer<LBFMRecommendListModel>.deserializeModelArrayFrom(json: json["list"].description){
                        self.recommendList = recomment as? [LBFMRecommendListModel]
                    }
