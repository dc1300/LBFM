...
class LBFMPlayDetailLikeCell: UITableViewCell {
    ...
    lazy var paidLabel: UILabel = {
        let paidLabel:UILabel = UILabel.init()
        paidLabel.font = UIFont.systemFont(ofSize: 13)
        paidLabel.textColor = UIColor.white
        paidLabel.backgroundColor = UIColor.init(r: 248, g: 210, b: 74)
        paidLabel.layer.masksToBounds = true
        paidLabel.layer.cornerRadius = 3
        paidLabel.textAlignment = NSTextAlignment.center
        return paidLabel
    }()
    ...
    var classifyVerticalModel:LBFMClassifyVerticalModel? {
        didSet{
            ...
            self.picView.kf.setImage(with: URL.init(string: model.coverMiddle!))
            if model.isPaid {
